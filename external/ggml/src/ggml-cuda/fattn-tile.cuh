...
static __global__ void flash_attn_tile(
    ...
    // MINITTS_FLASH_PER_HEAD_MASK:
    // The tile kernels can consume the same shared mask as upstream ggml, but
    // they can also index a per-head additive bias by wrapping head0 through
    // the mask head dimension.
    const half * maskh = mask ? (const half *) (mask + nb33*(sequence % ne33) + nb32*(head0 % ne32)) : nullptr;
