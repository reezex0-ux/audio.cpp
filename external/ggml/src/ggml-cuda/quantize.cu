...
// Fish S2 Phase 1O: exact input-round path for a metadata RESHAPE bridge.
// Keep the generic Q8_1 quantizer untouched; this dedicated kernel applies
// the same F32 -> BF16 -> F32 conversion as GGML_UNARY_OP_ROUND_BF16 before
// the existing Q8 reduction/quantization order.
__launch_bounds__(CUDA_QUANTIZE_BLOCK_SIZE, 1)
static __global__ void quantize_q8_1_round_bf16(
        const float * __restrict__ x, void * __restrict__ vy,
        const int64_t ne00, const int64_t s01, const int64_t s02, const int64_t s03,
        const int64_t ne0, const uint32_t ne1, const uint3 ne2) {
    const int64_t i0 = (int64_t)blockDim.x*blockIdx.x + threadIdx.x;
    ...
    const float xi_raw = i0 < ne00 ? x[i03*s03 + i02*s02 + i01*s01 + i00] : 0.0f;
    const float xi = __bfloat162float(__float2bfloat16(xi_raw));
    float amax = fabsf(xi);
    float sum = xi;
...
void quantize_row_q8_1_round_bf16_cuda(
        const float * x, const int32_t * ids, void * vy, const ggml_type type_src0,
        const int64_t ne00, const int64_t s01, const int64_t s02, const int64_t s03,
        const int64_t ne0, const int64_t ne1, const int64_t ne2, const int64_t ne3, cudaStream_t stream) {
    GGML_ASSERT(!ids);
    ...
    const dim3 num_blocks(block_num_x, ne1, ne2*ne3);
    const dim3 block_size(CUDA_QUANTIZE_BLOCK_SIZE, 1, 1);
    quantize_q8_1_round_bf16<<<num_blocks, block_size, 0, stream>>>(
